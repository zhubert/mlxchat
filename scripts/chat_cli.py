...
def get_args():
    ...
    # Model loading
    parser.add_argument("-c", "--checkpoint", type=str, default=None, help="Path to checkpoint directory")
    parser.add_argument("-i", "--source", type=str, default="base", help="Source of the model: base|sft|mid|rl")
    parser.add_argument("-g", "--model-tag", type=str, default=None, help='Model tag to load (e.g., "d12")')
    parser.add_argument("-s", "--step", type=int, default=None, help="Specific checkpoint step to load")

    # Generation parameters
    parser.add_argument(
        "-t", "--temperature", type=float, default=0.6, help="Temperature for generation (0.0 = greedy)"
    )
    parser.add_argument("-k", "--top-k", type=int, default=50, help="Top-k sampling parameter")
    parser.add_argument("-m", "--max-tokens", type=int, default=256, help="Maximum tokens to generate per response")

    # Single-shot mode
    parser.add_argument("-p", "--prompt", type=str, default="", help="Prompt the model once and exit")

    # Tokenizer
    parser.add_argument("--tokenizer-dir", type=str, default=None, help="Custom tokenizer directory")
...
def main():
    ...
    if args.checkpoint:
        ...
        model, tokenizer, metadata = load_model_from_dir(args.checkpoint, model_tag=args.model_tag, step=args.step)
        print(f"Loaded checkpoint: {args.checkpoint}")
    else:
        # Load from source (base/sft/mid/rl)
        try:
            model, tokenizer, metadata = load_model(source=args.source, model_tag=args.model_tag, step=args.step)
